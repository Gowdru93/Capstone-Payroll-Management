...
import { LEAVE_TYPES, ROLES } from '../../utils/constants'
...
const LeaveForm = () => {
  ...
  // Load employee (self) if not admin
  useEffect(() => {
    if (!user?.id) return
  ...
  useEffect(() => {
    if (formData.startDate && formData.endDate) {
      setLeaveDays(calculateLeaveDays(formData.startDate, formData.endDate))
    } else {
      setLeaveDays(0)
    }
            ...
            <div className="card-body text-center">
              ...
              {leaveDays > 0 && user?.role !== ROLES.ADMIN && (
                <div className="alert alert-info">
                  <small>
                    After this request: <strong>{employee.leaveBalance - leaveDays}</strong> days
                    remaining
                  </small>
                </div>
              )}
